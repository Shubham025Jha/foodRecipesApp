package com.upstox.android.foody.ui.fragments.foodjoke
...
import com.upstox.android.foody.databinding.FragmentFoodJokeBinding
        ...
        mainViewModel.foodJokeResponse.observe(viewLifecycleOwner) { response ->
            when (response) {
                is NetworkResult.Success -> {
                    binding.foodJokeTextView2.text = response.data?.text
        ...
        lifecycleScope.launch {
            mainViewModel.readFoodJoke.observe(viewLifecycleOwner) { database ->
                if (!database.isNullOrEmpty()) {
                    binding.foodJokeTextView2.text = database.first().foodJoke.text
